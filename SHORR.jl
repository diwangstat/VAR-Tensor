...
function SHORR(ts,lag_order,mtl_rank,n_lambda,init_value="MLR",ϵ=1e-6,max_itr=50)
    ...
    # lambda_path
    lambda_max = 1/(sum(abs.(U[1]))*sum(abs.(U[2]))*sum(abs.(U[3])))*0.1;
    ...
    for u_lambda = 1:n_lambda
        ...
        for i_admm = 1:max_itr
            ...
            U[1] = lasso_ortho_admm(reshape(Y,K*n,1),X_1,lambda_seq[n_lambda-u_lambda+1]*sum(abs.(U[2]))*sum(abs.(U[3])),mtl_rank[1],K);
            ...
            U[2] = lasso_ortho_admm(reshape(Y,n*K,1),X_2,lambda_seq[n_lambda-u_lambda+1]*sum(abs.(U[1]))*sum(abs.(U[3])),K,mtl_rank[2])';
            ...
            U[3] = lasso_ortho_admm(reshape(Y,n*K,1),X_3,lambda_seq[n_lambda-u_lambda+1]*sum(abs.(U[1]))*sum(abs.(U[2])),mtl_rank[3],P);
...
function SHORR2(ts,lag_order,mtl_rank,n_lambda,init_value="MLR",ϵ=1e-6,max_itr=50)
    ...
    # lambda_path
    lambda_max = 1/(sum(abs.(U[1]))*sum(abs.(U[2]))*sum(abs.(U[3])));
    lambda_seq = exp.(log(lambda_max).-log(n_lambda)/n_lambda*(0:n_lambda-1))/2;
    ...
    for u_lambda = 1:n_lambda
        ...
        for i_admm = 1:max_itr
            ...
            G = reshape(AO_LS(reshape(Y,n*K),X_g,mtl_rank),mtl_rank[1],mtl_rank[2],mtl_rank[3]);
